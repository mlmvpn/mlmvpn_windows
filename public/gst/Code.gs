...
// ── Optional Cloudflare Worker exit ─────────────────────────
// Leave EMPTY for the normal path: this script does the outbound fetch itself.
// Set it to your Worker URL to forward instead:
//
//   empty  →  client → Apps Script → target        (works with no Cloudflare at all)
//   set    →  client → Apps Script → Worker → target
//
// THIS ONE CONSTANT IS THE CLOUDFLARE SWITCH for this relay. The desktop app writes it
// for you; you only ever change it by re-pasting the script the app generates.
//
// Why forwarding is worth it: the outbound fetch (and the base64 encode of the body)
// stops happening inside Apps Script, so each call spends its time on one hop to the
// edge instead. Latency drops and the 90 min/day Apps Script runtime budget stretches.
// A batch of N URLs also becomes ONE UrlFetchApp call instead of N, because the Worker
// fans out internally — that is the bulk of the daily-quota saving.
//
// If the Worker is unreachable the script falls back to fetching directly, so a broken
// Worker degrades this relay to the normal path rather than taking it offline.
const WORKER_URL = "";
...
// ── Cloudflare Worker forwarding ───────────────────────────

function _workerEnabled() {
  return typeof WORKER_URL === "string" &&
         WORKER_URL.length > 0 &&
         WORKER_URL.indexOf("https://") === 0;
}

/**
 * Hand the whole payload to the Worker and return its answer verbatim.
 *
 * The Worker speaks the exact same wire protocol this script does, so the response can
 * be passed straight through with no re-encoding — that is what keeps the saving real
 * instead of just moving the work around.
 *
 * Returns null when forwarding did not produce a usable answer, which tells the caller
 * to fall back to fetching directly. Fail-open is deliberate: a relay that stops working
 * because its OPTIONAL accelerator broke would be a worse product than a slower one.
 */
function _forwardToWorker(payload) {
  try {
    var resp = UrlFetchApp.fetch(WORKER_URL, {
      ...
      // Long enough for a slow target, short enough to leave room to retry directly
      // inside the 6-minute Apps Script execution ceiling.
    ...
    // The Worker answers with the decoy HTML when the key is wrong. Detect that rather
    // than returning a page the client cannot parse.
    if (text.indexOf("<") === 0) return null;

    var parsed = JSON.parse(text);
    if (!parsed || (!parsed.s && !parsed.q && !parsed.e)) return null;
    return parsed;
  } catch (err) {
    return null;
  }
...
function doPost(e) {
  try {
    ...
    // Cloudflare path, when this relay has a Worker configured. One forward carries the
    // whole request — single or batch — so an N-URL batch costs 1 UrlFetchApp call.
    if (_workerEnabled()) {
      var forwardPayload = Array.isArray(req.q)
        ? { k: WORKER_AUTH_KEY, q: req.q }
        : { k: WORKER_AUTH_KEY, m: req.m, u: req.u, h: req.h, b: req.b, ct: req.ct, r: req.r };

      var forwarded = _forwardToWorker(forwardPayload);
      if (forwarded) return _json(forwarded);
      // else: fall through and fetch directly — see _forwardToWorker's note on fail-open.
    }
